...
function Index({ data }: any) {
  ...
  useEffect(() => {
    ...
    if (data?.path && typeof data?.path !== "string") {
      ...
      (async () => {
        try {
          const [res, status]: any = await uploadAPI.getFile(data?.path);
          if (res && status === 200) {
            setFileData(res);
          }
        } catch (err) {}
...
function LocationMarker({ data }: any) {
  const map = useMapEvents({});

  useEffect(() => {
    if (data?.area?.zoom && data?.area?.lat && data?.area?.lng) {
      map.flyTo([data?.area?.lat, data?.area?.lng], data?.area?.zoom);
    }
  }, [data, map]);
